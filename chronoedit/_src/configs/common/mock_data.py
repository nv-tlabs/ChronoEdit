...
from chronoedit._src.datasets.cached_replay_dataloader import get_cached_replay_dataloader
from chronoedit._src.datasets.data_sources.mock_data import get_image_dataset, get_video_dataset
from chronoedit._src.datasets.joint_dataloader import IterativeJointDataLoader
...
_VIDEO_LOADER = L(get_cached_replay_dataloader)(
    dataset=L(get_video_dataset)(
        resolution="512",
        num_video_frames=9,  # number of pixel frames, the number needs to agree with tokenizer encoder since tokenizer can not handle arbitrary length
    ),
    batch_size=1,
    shuffle=False,
    num_workers=8,
