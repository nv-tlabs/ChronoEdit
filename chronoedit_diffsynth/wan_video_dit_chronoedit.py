...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        self.rope_temporal_skip_len = rope_temporal_skip_len
    ...
    def forward(self,
        ...
        assert f == 2 or f == self.rope_temporal_skip_len, f"f must be 2 or {self.rope_temporal_skip_len}, but got {f}"
        if f == 2:
            freqs_f = self.freqs[0][:self.rope_temporal_skip_len][[0, -1]].view(f, 1, 1, -1).expand(f, h, w, -1)
        else:
        ...
        freqs = torch.cat([freqs_f, freqs_h, freqs_w], dim=-1).reshape(f * h * w, 1, -1).to(x.device)
...
class WanModelStateDictConverter:
    ...
    def from_diffusers(self, state_dict):
        rename_dict = {
            ...
            # "blocks.0.attn2.norm_added_q.weight": "blocks.0.cross_attn.norm_q_img.weight",
        ...
        elif hash_state_dict_keys(state_dict) in ["7cf3a086b49216bded0728ce78d59687", "e0e8b7e0ab5bbf15b88be20fdf9dfcbd"]:
            config = {
