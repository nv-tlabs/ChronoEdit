...
from chronoedit_diffsynth.wan_video_dit_chronoedit import WanModel
from chronoedit_diffsynth.model_manager import ModelManager
...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        # ChronoEdit
        enable_temporal_reasoning: Optional[bool] = False,
        num_temporal_reasoning_steps: Optional[int] = 0,
    ):
        # Scheduler
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if enable_temporal_reasoning and progress_id == num_temporal_reasoning_steps:
                inputs_shared['latents'] = inputs_shared['latents'][:, :, [0, -1]]
                inputs_shared['y'] = inputs_shared['y'][:, :, [0, -1]]

                # for j in range(len(self.scheduler.model_outputs)):
                #     if self.scheduler.model_outputs[j] is not None:
                #         if inputs_shared['latents'].shape[-3] != self.scheduler.model_outputs[j].shape[-3]:
        ...
        if enable_temporal_reasoning and num_temporal_reasoning_steps > 0:
            video_edit = self.vae.decode(inputs_shared["latents"][:, :, [0, -1]], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video_reason = self.vae.decode(inputs_shared["latents"][:, :, :-1], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
